...
def ind_col(frame):
    new_ind = pd.MultiIndex.from_tuples([tuple(x.split()) for x in list(set([x[1] for x in df.index.to_list()]))])
    ...
    for i in frame.iterrows():
        ...
        tempframe.at[tuple(i[0][1].split())] = val
        print(val)
        print(i)
    return tempframe
